...
export async function POST(request: NextRequest) {
  try {
    ...
    // إنشاء رمز تسجيل الدخول السحري
    const magicToken = uuidv4()
    const magicTokenExpiry = new Date(Date.now() + 900000) // 15 دقيقة

    console.log("Generated magic token for user:", user._id)

    // تحديث المستخدم برمز تسجيل الدخول السحري
    await client
      .patch(user._id)
      .set({
        magicToken,
        magicTokenExpiry: magicTokenExpiry.toISOString(),
      })
      .commit()
    ...
    const magicUrl = `${process.env.NEXTAUTH_URL}/magic-link?token=${magicToken}`

    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to: email,
      subject: "تسجيل الدخول - فذلكه",
      html: `
        <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: white; padding: 40px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <h2 style="color: #333; margin: 0 0 20px 0; font-size: 24px;">تسجيل الدخول</h2>
            ...
            <p style="color: #999; font-size: 14px; line-height: 1.6; margin: 30px 0 0 0;">
              هذا الرابط سينتهي صلاحيته خلال 15 دقيقة.
            </p>
