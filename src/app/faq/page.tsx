...
import { 
  ...
  FaUsers, FaGlobe, FaHeart, FaSearch, FaTimes, FaFilter, FaTag, FaBook, 
  FaGraduationCap, FaVideo, FaMicrophone, FaUserFriends, FaLightbulb as FaIdea, 
  FaCalendarAlt, FaPhone, FaWhatsapp, FaTelegram, FaFacebook, FaTwitter, 
  FaInstagram, FaYoutube, FaMapMarkerAlt, FaClock, FaHeadset, FaFileAlt,
  FaChevronRight, FaArrowRight, FaCheckCircle, FaInfoCircle, FaList, FaChartBar
} from "react-icons/fa";
        ...
        <div className="relative z-10 py-16 px-6 md:px-8 flex flex-col md:flex-row items-center justify-between">
          <div className="md:w-1/2 mb-12 md:mb-0 text-center md:text-right">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-6 leading-tight">
              الأسئلة <span className="text-yellow-300">الأكثر شيوعاً</span>
            </h1>
            <p className="text-lg md:text-xl text-blue-100 mb-8 max-w-lg mx-auto md:mx-0">
              إجابات على استفساراتكم حول قناتنا العلمية على يوتيوب وخدماتنا التعليمية
            </p>
        ...
        <div className="relative bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-3xl p-6 md:p-8 border border-indigo-100 dark:border-indigo-800 text-center overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-700 dark:text-gray-300 text-lg mb-8 max-w-2xl mx-auto">
              فريق الدعم متاح لمساعدتك في أي استفسار. لا تتردد في التواصل معنا للحصول على المساعدة التي تحتاجها.
            </p>
            <Link href="/contact">
              <motion.button
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div className="text-center md:text-right">
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 dark:text-gray-100 flex flex-col md:flex-row items-center justify-center md:justify-start">
                ...
                <span className="mt-2 md:mt-0">الأسئلة الشائعة</span>
              </h2>
              <p className="text-gray-600 dark:text-gray-400 mt-2 text-base md:text-lg">إجابات على استفساراتكم حول قناتنا العلمية على يوتيوب وخدماتنا التعليمية</p>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-10">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="w-full p-4 pr-12 pl-12 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:outline-none focus:ring-0 focus:border-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-lg transition-all duration-300 hover:border-indigo-300 dark:hover:border-indigo-600 focus:shadow-indigo-500/20"
                suppressHydrationWarning={true}
              />
              {searchTerm && (
                <button 
                  onClick={() => setSearchTerm("")}
                  className="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                >
                  <FaTimes />
                </button>
              )}
              ...
              {showSuggestions && searchTerm && (
                <div className="absolute z-40 mt-2 w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-y-auto">
            ...
            <div className="relative">
              <button
                ...
                suppressHydrationWarning={true}
                className="flex items-center justify-center w-full md:w-auto px-4 md:px-6 py-4 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:from-indigo-600 hover:to-purple-700 transform hover:-translate-y-0.5"
              >
                <FaFilter className="ml-2" />
                <span className="truncate">
                  {activeCategory ? activeCategory : "التصنيفات"}
                </span>
                {activeCategory && (
                  <span className="ml-2 bg-white text-indigo-600 text-xs rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0">
                    1
                  </span>
                )}
              </button>
              ...
              {showCategories && (
                <div className="absolute z-40 mt-2 w-full md:w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 max-h-60 overflow-y-auto">
                  <div 
                    className="p-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors duration-200 flex items-center justify-between"
                    onClick={() => {
                      setActiveCategory(null);
                      setShowCategories(false);
                    }}
                  >
                    <span className="font-medium text-gray-900 dark:text-gray-100">جميع التصنيفات</span>
                    {!activeCategory && (
                  ...
                  {categories.map((category) => (
                    ...
                    >
                      <div className="flex items-center min-w-0">
                        ...
                        <span className="ml-2 text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-2 py-1 rounded-full flex-shrink-0">
          ...
          {activeCategory && (
            <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-2">
              <div className="flex items-center">
                <span className="text-gray-600 dark:text-gray-400 ml-2">التصفية حسب:</span>
                <span className="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-sm font-medium truncate max-w-xs">
                  {activeCategory}
                </span>
              </div>
              <button 
                onClick={() => setActiveCategory(null)}
                className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 flex items-center flex-shrink-0"
              >
          ...
          {faqError && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6 text-center">
              ...
              <button 
                onClick={reloadFaqs}
                className="mt-4 px-6 py-3 bg-red-100 dark:bg-red-800 text-red-700 dark:text-red-200 rounded-xl hover:bg-red-200 dark:hover:bg-red-700 transition-colors font-medium"
              >
                إعادة المحاولة
              </button>
            </div>
          ...
          {!faqLoading && !faqError && filteredFaqs.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-gray-500 dark:text-gray-400">
                {searchTerm || activeCategory ? "لا توجد أسئلة تطابق بحثك أو الفئة المحددة" : "لا توجد أسئلة شائعة حالياً"}
              </p>
              {(searchTerm || activeCategory) && (
                <div className="mt-4 flex flex-wrap justify-center gap-3">
                  {searchTerm && (
                    <button 
                      onClick={() => setSearchTerm("")}
                      className="px-6 py-3 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 rounded-xl hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors font-medium"
                    >
                      مسح البحث
                    </button>
                  )}
                  {activeCategory && (
                    <button 
                      onClick={() => setActiveCategory(null)}
                      className="px-6 py-3 bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-200 rounded-xl hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors font-medium"
                    >
                      إلغاء الفلتر
                    </button>
                  )}
              ...
              {filteredFaqs.map((f, index) => {
                ...
                return (
                  <motion.div 
                    ...
                    className={`overflow-hidden rounded-2xl bg-white dark:bg-gray-800 transition-all duration-300 ${
                      isOpen 
                        ? `ring-2 ring-opacity-50 ${colors.ring} dark:shadow-lg ${colors.shadow} ${colors.shimmer}` 
                        : `${colors.shadow} ${colors.hoverShadow}`
                    }`}
                    style={{
                      borderLeft: `4px solid var(--card-${colorIndex}-border)`,
                      borderTop: `1px solid var(--card-${colorIndex}-border)`,
                      borderRight: `1px solid var(--card-${colorIndex}-border)`,
                      borderBottom: `1px solid var(--card-${colorIndex}-border)`,
                      boxShadow: isOpen 
                        ? `0 10px 25px -5px var(--card-${colorIndex}-shadow-color), 0 10px 10px -5px var(--card-${colorIndex}-shadow-color), 0 0 15px var(--card-${colorIndex}-shimmer-color)` 
                        : `0 10px 25px -5px var(--card-${colorIndex}-shadow-color), 0 10px 10px -5px var(--card-${colorIndex}-shadow-color)`
                    }}
                  >
